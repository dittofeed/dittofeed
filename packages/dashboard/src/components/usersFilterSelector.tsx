...
import { FilterOptions, filterIds, filterStore } from "../lib/filterStore";
...
const IdAndValueSelector = ({
    ...
    const setPropertiesValues = filterStore(
...
const SelectorFooter = ({
    ...
    return (
      <Box paddingX="5%" textAlign="left" height="18px" display='flex' justifyContent="space-between" alignItems="center" mt="5px">
        <KeyboardBackspaceIcon sx={{ width: "15px", cursor: "pointer"}} onClick={() => handlePrevious()}/>
        {selectedFilter === FilterOptions.USER_PROPERTY && stage === Stage.SELECTING_VALUE && filter !== ''
            ? <Typography 
...
export default function FilterSelect({ workspaceId }: { workspaceId: string }) {
  ...
  const setSelectedId = filterStore(
  ...
  const handleValueSelection = (propertyAssignmentId: string | undefined, isPartialMatch?: boolean) => {
      setFilter('')
  ...
  return (
    <div>
      ...
      > 
       <Box maxHeight="200px" minWidth="150px" maxWidth="150px" overflow="scroll">
