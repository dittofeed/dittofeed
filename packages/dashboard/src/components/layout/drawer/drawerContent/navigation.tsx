...
function Navigation() {
  ...
  const navGroups = items ? items.map((item) => {
    switch (item.type) {
      case "group":
        return isMinimal ? <MinimalNavGroup key={item.id} item={item} /> : <NavGroup key={item.id} item={item} />;
      default:
        return (
      ...
      {title ?
        (
          <Stack direction="row" sx={{ ml: -1 }}>
            ...
            <Typography mb={2} variant="h1" fontSize={21}>{title}</Typography>
          </Stack>
        ) : null}
      {navGroups}
    </Box>);
