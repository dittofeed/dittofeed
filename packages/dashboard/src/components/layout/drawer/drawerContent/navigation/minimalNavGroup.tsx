...
function MinimalNavGroup({ item }: { item: MenuItemGroup }) {
  ...
  return (hasChildren ?
    <List sx={{ mb: drawerOpen ? 1 : 0, py: 0, zIndex: 0 }}>
      <ListItemButton onClick={toggleExpand} sx={{ paddingX: 1, paddingY: 0.5, borderRadius: 2 }}>
    ...
    <ListItemButton href={item.url ?? ""} component={item.external ? "a" : Link} sx={{ paddingX: 1, paddingY: 0.5, borderRadius: 2, mb: drawerOpen ? 1 : 0, zIndex: 0 }}>
      <ListItemText primary={item.title} />
    </ListItemButton>
  );
}
