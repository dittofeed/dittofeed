...
1. Create a webhook message template. Note that in the example below we use a secret to store the API key.
...
2. Create a user property that renders the webhook response. This user property should condition on the `DFInternalMessageSent` event name, with the `variant.response.body` path. Furthermore, it should check for the `templateId` property to be equal to the `templateId` of the webhook message template you created e.g. `edfc6da5-f276-4937-9519-427e3164d9d8`.
