...
Users can be added to subscription groups with manual csv uploads form the dashboard. To do so navigate to a particular subscription group's upload page `/dashboard/subscription-groups/upload/[id]`.
...
The subscription management will either unsubscribe, or subscribe the user to a specific users to a subscription group on page load. Unsubscribe links link to the subscription management page with the unsubscribe action.
...
![Subscription Management Preview](/images/subscription-management-preview.png)


## Adding Subscription Groups to a User Journey

In order to use a subscription group, it must be added to a user journey. This ensures that users are not messaged from message nodes unless they are in subscription group at time of send.

{/* FIXME image of adding subscription group */}

It also allows unsubscribe links to be rendered in emails.

## Adding Unsubscribe Links to Emails

When journey message nodes have subscription groups associated with them, unsubscribe links can be added to emails.

{/* FIXME add screenshot of template */}
{/* FIXME add code snippet */}

Users clicking on the page will be directed to the subscription management page, where will be unsubscribed from the subscription group on page load.
...
It's possible to view which users are in a subscription group by viewing the subscription group's user page in the dashboard `/dashboard/subscription-groups/users/[id]`.

{/* FIXME add screenshot of subscription group user page */}
