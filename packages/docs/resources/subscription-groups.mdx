...
<Note>
For deployments with restricted ingress (e.g., only exposing `/api/public/*`), Dittofeed provides a self-contained subscription management page that can be served directly from the API. See [Custom Subscription Management Page](/guide/custom-subscription-page) for details.
</Note>
