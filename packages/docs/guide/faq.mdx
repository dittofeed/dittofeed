...
4. 5 minutes pass, and the specified segment window expires.
5. Dittofeed processes the segment window expiration and removes the user from the `loggedIn` segment.

At this point the user is no longer in the `loggedIn` segment, and any `LOGGED_IN` events will re-assign the user to the segment and re-start the journey.
