...
```bash
AUTH_MODE=multi-tenant IMAGE_TAG=v0.22.0-ee.3 IMAGE_REPOSITORY=dittofeed/dittofeed-ee docker-compose -f docker-compose.lite.yaml up -d
```

Then, uncomment the block in the docker-compose.lite.yaml file that looks like this to include secret environment variables:
...
```bash
helm install dittofeed dittofeed/dittofeed --set image.repository=dittofeed/dittofeed-ee --set image.tag=v0.22.0-ee.3
```
