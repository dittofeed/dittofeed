---
title: "Upgrading from v0.11.0 to v0.12.0"
---

<Warning> v0.12.0 is currently in alpha, upgrade at your own risk</Warning>

Upgrading from v0.11.0 to v0.12.0 is fairly straightforward. This guide will walk you through the process of upgrading your Dittofeed instance.
...
services:
  lite:
    image: dittofeed/dittofeed-lite:${IMAGE_TAG:-v0.11.0}
    restart: always
    ports:
      - "3000:3000"
    depends_on:
  ...
  admin-cli:
    image: dittofeed/dittofeed-lite:${IMAGE_TAG:-v0.12.0}
    entrypoint: []
    profiles: ["admin-cli"]
...
Run the upgrade script before deploying the new version of Dittofeed:

```bash
./scripts/admin-lite.sh upgrade-0-12-0-pre
```

## Step 4: Upgrade Your `docker-compose.lite.yaml` File

Upgrade the version in your docker-compose.lite.yaml file to v0.12.0:
