...
function unwrapTag(tagName: string, tags: Record<string, any>) {
...
export function postMarkEventToDF({
    ...
    const { RecordType: event, Metadata, DeliveredAt, ReceivedAt, BouncedAt, Email, Recipient } = postMarkEvent;
